...
class PasteStorage {
    ...
    private constructor() {
        // Clean up expired pastes every 30 seconds
        setInterval(() => {
    ...
    async create(paste: Paste): Promise<void> {
        ...
        console.log(`[STORAGE] Created paste: ${paste.id}, Total: ${this.pastes.size}`);
    }

    async get(id: string): Promise<Paste | null> {
        const paste = this.pastes.get(id);
        if (!paste) {
            console.log(`[STORAGE] Paste not found: ${id}`);
            return null;
        }

        // Check if expired
        if (paste.expires_at && paste.expires_at < Date.now()) {
            console.log(`[STORAGE] Paste expired: ${id}`);
            this.pastes.delete(id);
            return null;
        }
    ...
    async update(id: string, updates: Partial<Paste>): Promise<void> {
        ...
        if (existing) {
            ...
            console.log(`[STORAGE] Updated paste: ${id}, Views: ${updates.views}`);
        }
    }
    ...
    // Debug method
    debug() {
        console.log(`[STORAGE DEBUG] Total pastes: ${this.pastes.size}`);
        for (const [id, paste] of this.pastes.entries()) {
            console.log(`  ${id}: ${paste.content.substring(0, 50)}...`);
        }
    }
}
